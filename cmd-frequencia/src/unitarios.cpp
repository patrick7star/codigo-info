...
extern "C" void executa_testes_b (uint8_t total, ...);
...
void outlies_que_contabilizam_quase_tudo(void)
{
   ssize_t acumulado{0}, cursor{0};
   double percentual{0.0};

   ifstream arquivo(CAMINHO, ios::in);
   auto distribuicao = computa_frequencia(arquivo); 
   arquivo.close();

   // Obtendo o acumulado.
   for (auto it = distribuicao.cbegin(); it != distribuicao.cend(); ++it)
   {
   ...
   {
      if (percentual < 0.90) {
         percentual += (double)it->second;
         cursor++;
      } else
         break;

      cout << "Percentual em " << cursor << '/' 
           << percentual * 100.0 << '%' << endl;
   }
...
int main (void) {
   executa_testes_b (
      true, 1, 
         Unit(outlies_que_contabilizam_quase_tudo, true)
   );
   executa_testes (

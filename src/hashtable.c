...
char* ht_to_str(HashTable m) {
   ...
   // valor grande inicial, ainda precisa ser computado o tamanho real.
   char* string = malloc (1000 * sizeof(char));

   // zera cada caractére na array para sucessivas concatenações.
   memset (string, '\0', 1000);
   // concatena o cabeçalho da string.
   strcat (string, "Hashtable: {");
   ...
   for (size_t y = 0; y < capacidade; y++) {
      for (size_t x = 0; x < colunas; x++) {
         ...
         if (!chaves_batem(atual, CHAVE_VAZIA)) {
            // conversão da chave e valor para string ...
            Entrada e = cria_entrada (atual->chave, atual->valor);
            char* entradastr = entrada_to_str(e);
            // concatenando...
            strcat (string, entradastr);
            strcat (string, ", ");
         }
      }
   }
   // finaliza e retorna a construção.
   strcat (string, "\b\b}");
   return string;
}
...
void main() {
   executa_testes(
      ...
      conversao_da_hastable_em_string, true
   );
   return;
}
#endif
